...
begin
  ...
  with latest_runs as (
    select distinct on (ar.workflow_key)
      ...
      ar.source_job_id,
      case
        when ar.source_job_id is not null then ar.source_job_id
        when ar.workflow_key in ('competitor_discovery', 'faq_generation')
          and coalesce(ar.input_snapshot ->> 'competitor_research_job_id', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then (ar.input_snapshot ->> 'competitor_research_job_id')::uuid
        else null
      end as resolved_source_job_id,
      ar.input_snapshot,
      ar.output_summary,
      ar.error_summary,
      ar.started_at,
    ...
    'tracks', jsonb_build_object(
      ...
      coalesce(
        (
          select jsonb_build_object(
            ...
            'job_id', (lr.resolved_source_job_id)::text,
          ...
          from latest_runs lr
          left join latest_competitor_job lcj on lcj.id = lr.resolved_source_job_id
      ...
      coalesce(
        (
          ...
          from latest_runs lr
          left join latest_scraping_job lsj on lsj.id = lr.source_job_id
        ...
        (
          select jsonb_build_object(
            ...
            'counts', jsonb_build_object(
              ...
              'faqs_added', coalesce(lcj.faqs_added, (lr.output_summary ->> 'faq_count')::integer, 0)
            ),
            'latest_error', coalesce(lr.error_summary ->> 'reason', lcj.error_message),
            'started_at', lr.started_at,
            'completed_at', lr.completed_at,
