-- Extend the unified pipeline to cover the full live inbound channel set
-- and keep social / Google Business identities stable inside the same sorter.
...
begin
  ...
  if v_customer_id is null then
    ...
    values (
      ...
      case
        when v_identifier_type in ('phone', 'whatsapp', 'facebook', 'instagram', 'google_business')
          then v_counterparty_norm
        else null
      end,
      v_event.channel,
      now()
...
begin
  ...
  if p_channel not in ('email','whatsapp','sms','facebook','instagram','google_business','voice') then
