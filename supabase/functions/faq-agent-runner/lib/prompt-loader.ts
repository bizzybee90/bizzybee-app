...
`,
  'website-faq-extraction.md': `You are extracting FAQs from the business's OWN website — one page at a time. These are the most authoritative answers because they come directly from the business owner.
...
If single_page_site = false:
  ...
  → Page-type guidance:
    - homepage: core USP, high-level service overview, coverage summary. Skip detail belonging on dedicated pages.
    - service: questions specific to THAT service only. Do NOT restate brand-level facts (insurance, guarantee, company experience).
    - location: ONLY location-specific facts (streets/areas covered within that town, location-specific pricing if different, local testimonials). DO NOT restate generic service facts that appear on every location page. Most location pages should produce 0–2 FAQs, not 6. Zero is fine.
    - pricing: pricing, packages, minimums, billing, discounts.
    - about: insurance, guarantees, experience, accreditations, reviews, company trust signals.
    - faq: extract every relevant question without exception. This page IS the natural home for generic facts.
    - contact: usually nothing. Skip unless it contains genuine FAQs.
    - blog: only the specific question the post answers, if any.
    - product: product-specific details, variants, what's included, specifications.
    - menu: dish/item specifics, pricing, dietary info.
    - policy: only the specific policy content (returns, shipping, privacy, etc.).
    - other: extract conservatively — only facts clearly tied to this page's specific content.
