...
function OutcomeBadge({ outcome }: { outcome: string | null }) {
  if (!outcome) return <span className="text-[var(--text-secondary)]">--</span>;
    ...
    <tr>
      <td colSpan={7} className="px-4 py-4 bg-[var(--bg-secondary)]">
        <div className="space-y-4 max-w-3xl">
          {/* Transcript */}
          <CallTranscript
            transcriptObject={call.transcript_object as any}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            {call.outcome_details && Object.keys(call.outcome_details).length > 0 && (
              <div>
                <p className="text-[var(--text-secondary)] text-xs mb-0.5">Outcome Details</p>
                <p className="text-[var(--text-primary)]">
                  {JSON.stringify(call.outcome_details)}
                </p>
              </div>
            )}
            <div>
              <p className="text-[var(--text-secondary)] text-xs mb-0.5">Cost</p>
            ...
            {call.followup_notes && (
              <div className="col-span-2">
                <p className="text-[var(--text-secondary)] text-xs mb-0.5">Follow-up Notes</p>
                <p className="text-[var(--text-primary)]">{call.followup_notes}</p>
              </div>
            )}
          </div>
...
export const CallLogTable = () => {
  ...
  return (
    <div className="bg-white border border-[var(--separator)] rounded-xl overflow-hidden">
      {/* Filter bar */}
      <div className="flex flex-wrap items-center gap-3 p-4 border-b border-[var(--separator)]">
        <Select
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--separator)] text-left">
                <th className="px-4 py-2.5 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">
                  Time
                </th>
                <th className="px-4 py-2.5 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">
                  Caller
                ...
                <th className="px-4 py-2.5 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider w-12">
                  Mood
                </th>
                <th className="px-4 py-2.5 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">
                  Summary
              ...
              {calls.map((call) => {
                ...
                return (
                  <Fragment key={call.id}>
                    <tr
                      className={cn(
                        'border-b border-[var(--separator)] hover:bg-gray-50/50 cursor-pointer transition-colors',
